...
audio: "https://frontcore.t3.storage.dev/audios/frontend/rendering-and-browser-pipeline/render-blocking-resources.m4a"
image: "https://frontcore.t3.storage.dev/Images/frontend/rendering-and-browser-pipeline/render-blocking-resources.png"
---
...
`@import` inside CSS is particularly costly — the browser only discovers the imported file _after_ downloading and parsing the parent stylesheet, creating a sequential chain of blocking downloads.

### Why JavaScript Is Blocking by Default

A classic `<script>` tag without attributes does three things that hurt the CRP:

1. **Pauses the HTML parser** — the browser stops building the DOM the moment it hits the tag
2. **Waits for pending CSS** — because JS can call `getComputedStyle()`, the engine must ensure the CSSOM is complete before executing
3. **Executes synchronously** — the script runs in full before the parser resumes
...
defer               ✅ Yes         ✅ Yes                      ✅ Yes
type="module"       ✅ Yes         ✅ Yes (implicit defer)     ✅ Yes
```

`type="module"` behaves like `defer` by default — non-blocking and executes after parse. It also supports `async` as an explicit attribute for module scripts that should execute as soon as downloaded.
...
### Deferring Non-Critical CSS — The `media` Trick
...
<Callout type="warn">
  ...
  as styles apply after load — trading a blocking delay for a visible layout
...
```html
<!-- ✅ Parallel — browser discovers all three at the same time from HTML -->
<link rel="stylesheet" href="/styles/main.css" />
<link rel="stylesheet" href="/styles/typography.css" />
<link rel="stylesheet" href="/styles/components.css" />
```

For bundled projects (Next.js, Vite, webpack), the bundler collapses `@import` chains into a single output file automatically — so this is mainly a concern for unbundled stylesheets or third-party CSS that uses `@import` internally.
...
Preloading is appropriate only for the LCP candidate — typically one hero image. Preloading every image sends conflicting high-priority signals and causes bandwidth contention that worsens overall load performance. Preload one; let the browser prioritize the rest.
