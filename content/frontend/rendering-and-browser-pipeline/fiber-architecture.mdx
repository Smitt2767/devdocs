...
description: React's internal reconciliation engine — fiber nodes, double buffering, the work loop, how hooks are stored, and why the render phase must be pure.
audio: "https://frontcore.t3.storage.dev/audios/frontend/rendering-and-browser-pipeline/fiber-architecture.m4a"
image: "https://frontcore.t3.storage.dev/Images/frontend/rendering-and-browser-pipeline/fiber-architecture.png"
---
...
### Double Buffering — Two Trees, One Swap
...
1. **Before mutation** — `getSnapshotBeforeUpdate` for class components
2. **Mutation** — applies all DOM insertions, updates, and deletions
3. **Layout** — fires `useLayoutEffect` cleanup and setup synchronously
...
// Use only for debugging in development — never rely on this in production code.
```
...
The virtual DOM (React elements returned by JSX) is a description of what to render — lightweight objects created on every render pass. Fiber nodes are the persistent data structures that survive across renders, storing committed state, hooks, effects, and scheduling metadata. React elements are ephemeral; fibers are persistent.
