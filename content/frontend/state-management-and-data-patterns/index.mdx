...
The section builds from first principles — what state is and where it should live — before covering remote data, then the edge cases that only appear in production.

## What's covered

**Derived State vs Redundant State** — The single source of truth principle: when to compute state on the fly versus store it, and how redundant state causes sync bugs. Start here — it's the most commonly violated rule in React codebases.
...
**Data Fetching Patterns** — The mental model behind SWR and React Query: cache keys, deduplication, background revalidation, optimistic mutations, and when to use a client cache versus a Server Component fetch.
