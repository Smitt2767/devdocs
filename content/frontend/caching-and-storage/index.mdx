...
**HTTP Caching Strategies** — `Cache-Control` directives (`max-age`, `stale-while-revalidate`, `no-store`), `ETag` and `Last-Modified` for conditional requests, and how browser cache, service worker cache, and CDN cache layer on top of each other.
