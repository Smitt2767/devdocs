...
**HTTP Caching Strategies** — `Cache-Control` directives (`max-age`, `stale-while-revalidate`, `no-store`), `ETag` and `Last-Modified` for conditional requests, and when to use each strategy.

**HTTP Caching Hierarchy** — How browser cache, service worker cache, and CDN cache layer on top of each other, and which one wins in each scenario.
