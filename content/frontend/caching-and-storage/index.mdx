...
**Offline Conflict Resolution** — Storage-layer mechanics: IndexedDB write queue with `idb` (sequential flushing, put-based deduplication), Background Sync API for reconnection-triggered flush with `online` event fallback, `navigator.storage.persist()` to protect queue survival, and `BroadcastChannel` for SW-to-page conflict notification. Server-side optimistic locking with version numbers (409 conflict response). Three-way merge (`base + local + remote`) for auto-resolving non-overlapping field changes. Conflict resolution UI component for user-prompted resolution of overlapping edits.
