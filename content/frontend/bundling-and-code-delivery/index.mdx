...
**Module Federation** — Sharing code between independently deployed applications at runtime. Webpack 5 `ModuleFederationPlugin` configuration for remotes and hosts, `remoteEntry.js` mechanics, `shared` singleton configuration to prevent duplicate React instances, TypeScript type contracts via `declare module` and shared types packages, dynamic remote URL resolution at runtime, Vite plugin federation, and error boundaries for isolated remote failures.
