---
title: Overview
description: Patterns and mechanisms for structuring the UI layer — from RSC composition and Suspense to compound components, headless patterns, portal layering, Shadow DOM, edge rendering, and micro-frontend orchestration.
---
...
**Shadow DOM** — Native browser-level style and DOM encapsulation. `<slot>` for content projection. `::part()` for exposing internal elements to external CSS. `adoptedStyleSheets` for efficient shared stylesheets. Declarative Shadow DOM for SSR-compatible shadow roots. CSS custom properties as the theming API across the boundary.

**Edge Rendering** — The WinterCG runtime API surface, what's available and what isn't. Next.js middleware (always edge) vs edge route handlers (`export const runtime = 'edge'`). Streaming from the edge. `stale-while-revalidate` for ISR-style freshness. JWT verification with Web Crypto. When edge rendering beats static generation or Node.js SSR.

**Micro-frontend Orchestration** — Build-time vs server-side vs client-side (Module Federation) composition. The `remoteEntry.js` manifest and runtime module resolution. Singleton shared dependencies and the multiple-React-instances problem. Cross-remote communication via custom event bus (not shared stores). TypeScript remote module declarations. Platform contracts for version coordination. Integration testing across team boundaries.
