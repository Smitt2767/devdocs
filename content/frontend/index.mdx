...
Topics include: hydration, partial and selective hydration, streaming SSR, concurrent rendering, React's fiber architecture and reconciliation algorithm, the critical rendering path, render-blocking resources, browser compositing layers, paint vs composite vs layout, render waterfalls, the View Transitions API, and image and font optimization.
...
Topics include: Suspense boundaries, React Server Components, edge rendering, micro-frontend orchestration, Shadow DOM, error boundaries, compound components, the headless UI pattern, and portal and layering architecture.
...
Topics include: immutable data patterns, memoization pitfalls, race conditions in UI state, optimistic UI and rollback strategy, offline conflict resolution, idempotent UI actions, derived state vs redundant state, state boundaries, URL as state, and data fetching patterns.
...
Topics include: tree shaking internals, code splitting strategies, module federation, preload / prefetch / preconnect / priority hints, bundle analysis and dependency auditing, and monorepo tooling.
...
Topics include: HTTP caching strategies, cache invalidation strategies, IndexedDB, service worker lifecycle traps, offline conflict resolution, and CDN cache purging.
...
Topics include: Interaction to Next Paint, Cumulative Layout Shift, Largest Contentful Paint, Lighthouse CI integration, RUM vs synthetic monitoring, performance budgets, and third-party script management.
...
Topics include: browser memory leak detection, detached DOM nodes, garbage collection timing, and memory profiling with Chrome DevTools.
...
Topics include: framework selection criteria, technical debt identification and prioritization, Architecture Decision Records (ADRs), and API contract design.
